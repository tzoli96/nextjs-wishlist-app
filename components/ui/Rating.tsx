import { MAX_RATING } from '@/lib/constants'
import { formatRating, range } from '@/lib/utils'

interface RatingProps {
  rating: number
  count?: number
  showCount?: boolean
  size?: 'sm' | 'md' | 'lg'
}

const sizeClasses = {
  sm: 'h-4 w-4',
  md: 'h-5 w-5',
  lg: 'h-6 w-6',
}

export default function Rating({
  rating,
  count,
  showCount = true,
  size = 'md',
}: RatingProps) {
  const sizeClass = sizeClasses[size]

  const ariaLabel = count !== undefined
    ? `Rated ${formatRating(rating)} out of ${MAX_RATING} stars based on ${count} reviews`
    : `Rated ${formatRating(rating)} out of ${MAX_RATING} stars`

  return (
    <div className="flex items-center">
      <div
        className="flex items-center"
        role="img"
        aria-label={ariaLabel}
      >
        {range(MAX_RATING).map((i) => (
          <svg
            key={i}
            className={`${sizeClass} ${
              i < Math.floor(rating)
                ? 'text-yellow-400 fill-yellow-400'
                : 'text-gray-300 fill-gray-300'
            }`}
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
            aria-hidden="true"
          >
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
        ))}
      </div>
      {showCount && count !== undefined && (
        <span className="ml-2 text-sm text-gray-600" aria-hidden="true">
          {formatRating(rating)} ({count})
        </span>
      )}
      {showCount && count === undefined && (
        <span className="ml-2 text-sm text-gray-600" aria-hidden="true">
          {formatRating(rating)}
        </span>
      )}
    </div>
  )
}
